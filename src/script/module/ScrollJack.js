...
class ScrollJack {
  ...
  wheelHandler(ev) {
    ...
    let delta = oriEv.deltaY ? -(oriEv.deltaY) : oriEv.wheelDelta ? oriEv.wheelDelta : -(oriEv.detail);
    let deltaAbs = Math.abs(delta);

    // Safariだとdeltaが小さいので無理矢理でっかくする
    if (this.ua() === 'safari') {deltaAbs = deltaAbs * 151}
...
touchHandler() {
   ...
   */
  sectionPrev() {
    let _section = this.section = (this.section === this.sectionStart) ? this.sectionStart : this.section-1;
   ...
   */
  sectionNext() {
    let _section = this.section = (this.section === this.sectionEnd) ? this.sectionEnd : this.section+1;
    this.sectionMove(_section);
  }
  ...
  sectionMove(_section) {
    this.$body.removeClass((index, className) => {return (className.match(/\bis-\S+/g) || []).join(' ')}).addClass('is-'+_section);
  ...
  sectionRestoration(section) {
    ...
    let interval = setInterval(()=> {
      this.$body.removeClass((index, className) => {return (className.match(/\bis-\S+/g) || []).join(' ')}).addClass('is-'+this.section);
    }, 1);
